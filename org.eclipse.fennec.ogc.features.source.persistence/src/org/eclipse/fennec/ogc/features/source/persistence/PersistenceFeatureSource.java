...
/**
 * Translates a {@link FeatureQuery} into Fennec query IR queries ({@link Cql2ToQuery}) and
 * leaves only their execution to the subclass.
 * <p>
 * Without a residual, paging and counting are pushed to the store. With one, the
 ...
 */
public abstract class PersistenceFeatureSource implements FeatureSource {

	@Override
	public FeatureResult query(FeatureQuery query) {
		CollectionDescriptor collection = query.collection();
		Translation translation = query.filter() == null ? new Translation(null, null)
				: Cql2ToQuery.translate(query.filter(), collection);
		QueryBuilder builder = from(collection, translation);
		...
		if (translation.hasResidual()) {
			List<EObject> matching = objects(builder.build()).stream().filter(translation.residual()::matches).toList();
		...
		if (query.count()) {
			numberMatched = count(from(collection, translation).countOnly().build());
		}
	...
	private static QueryBuilder from(CollectionDescriptor collection, Translation translation) {
		QueryBuilder builder = QueryBuilder.from(collection.type());
		return translation.store() == null ? builder : builder.where(translation.store());
	}
