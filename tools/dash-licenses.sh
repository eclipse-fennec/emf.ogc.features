...
Options:
  --all                 Send every artifact of the repository index to Dash,
                        instead of only those the workspace references.
...
DEPS="$CACHE/deps.txt"
DEPS_ALL="$CACHE/deps-all.txt"
SKIPPED="$CACHE/deps-not-referenced.txt"
...
( cd "$ROOT" && java -jar "$BND_JAR" repo deps -o "$DEPS_ALL" )
ALL_COUNT="$(wc -l < "$DEPS_ALL" | tr -d ' ')"

# ---- 1b) keep only what the workspace actually asks for --------------------
# `bnd repo deps` exports every artifact the Maven repository INDEX offers, not
# the ones this workspace uses: jakarta.ws.rs, Jackson, ICU4J, the Gecko bnd
# libraries of other projects, Felix web console. Three quarters of the list is
# other people's shelf. Sending that to Dash asks the Eclipse IP team to vet
# software we do not ship, and the unvetted rest turns the check red for reasons
# that have nothing to do with us.
#
# An artifact counts as used when its artifactId appears anywhere in the bnd
# files of the workspace — that is where -buildpath, -testpath, -runbundles,
# -runfw and -runpath name their bundles. The match is deliberately a substring:
# the artifactId and the bundle symbolic name are not always the same word
# (org.antlr:antlr4 is required as org.antlr.antlr4-runtime), and over-including
# is the safe direction for a license check.
#
# What was dropped is written next to the list, so the decision stays reviewable
# rather than silent. Pass --all to skip this step and send everything.
if [[ "$FILTER" == true ]]; then
  echo ">> Reducing to the artifacts this workspace references ..."
  find "$ROOT" -name 'bnd.bnd' -o -name '*.bndrun' -o -name '*.bnd' \
    | grep -v '/cnf/cache/' | grep -v '/node_modules/' \
    | xargs cat > "$CACHE/bnd-files.txt" 2>/dev/null || true
  : > "$DEPS"
  : > "$SKIPPED"
  while IFS= read -r gav; do
    [[ -z "$gav" ]] && continue
    artifact="$(printf '%s' "$gav" | cut -d: -f2)"
    if grep -qF -- "$artifact" "$CACHE/bnd-files.txt"; then
      printf '%s\n' "$gav" >> "$DEPS"
    else
      printf '%s\n' "$gav" >> "$SKIPPED"
    fi
  done < "$DEPS_ALL"
  echo ">> $(wc -l < "$SKIPPED" | tr -d ' ') of $ALL_COUNT artifacts are in the repository index but not"
  echo "   referenced by any bnd file; they are listed in $SKIPPED"
else
  cp "$DEPS_ALL" "$DEPS"
fi
