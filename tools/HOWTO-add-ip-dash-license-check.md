...
| ~~`tools/dash-licenses.bat`~~ | The Windows runner is **not** carried in this repository: the filtering step below exists only in the shell script, and two runners that disagree about what they send to Dash are worse than one. Copy it from emf.osgi and port the filter if Windows support is ever needed. |
...
| ~~`.gitlab-ci.yml`~~ | Not carried here either — this project builds on GitHub Actions only. |
| `docs/ip-dash-license-check.md` | User-facing docs in emf.osgi; here the README and `NOTICE.md` point at `DEPENDENCIES` instead. |

### What this repository added: only what the workspace uses

`bnd repo deps` exports every artifact in the Maven repository *index*, which here is 200
artifacts — jakarta.ws.rs, Jackson, ICU4J, Felix web console, the Gecko bnd libraries of other
projects. 145 of them are not referenced by any bnd file in this workspace. Sending those to Dash
asks the Eclipse IP team to vet software we neither ship nor build with, and every unvetted one
turns the check red for a reason that is not ours.

The script therefore keeps only the artifacts whose artifactId appears somewhere in the
workspace's bnd files — that is where `-buildpath`, `-testpath`, `-runbundles`, `-runfw` and
`-runpath` name what they need. The match is a substring on purpose: artifactId and bundle
symbolic name are not always the same word (`org.antlr:antlr4-runtime` is required as
`org.antlr.antlr4-runtime`), and over-including is the safe direction for a license check. What
was dropped lands in `cnf/cache/dash-licenses/deps-not-referenced.txt` so the decision can be
reviewed instead of trusted. `--all` sends the unfiltered index.
