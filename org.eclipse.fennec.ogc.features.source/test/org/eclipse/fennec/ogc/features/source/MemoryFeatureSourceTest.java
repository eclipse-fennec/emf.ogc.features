...
class MemoryFeatureSourceTest {
	...
	@Test
	void bboxIsPushedAsEnvelopeComparison() {
		FeatureQuery query = FeatureQuery.builder(all).where(all.bbox().intersects(new Envelope(0.5, 0.5, 3.5, 2)))
				.count(true).build();
		FeatureResult result = source.query(query);
		assertThat(ids(result)).containsExactly("k1", "p1", "p2");
		assertThat(result.numberMatched()).isEqualTo(3);
	}
	...
	void propertyFilter() {
		FeatureQuery query = FeatureQuery.builder(pools)
				.where(path(BathPackage.Literals.POOL__POOL_TYPE).eq(PoolType.KIDS)).build();
		assertThat(ids(source.query(query))).containsExactly("p2");
	}

	@Test
	void residualIsAppliedBeforePagingAndCounting() {
		FeatureQuery query = FeatureQuery.builder(pools)
				.residual(o -> !((Pool) o).getId().equals("p1"))
